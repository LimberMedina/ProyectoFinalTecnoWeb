...
const hydrateSelections = () => {
    productosSeleccionados.value = (props.promocion.productos || []).map(
        (producto) => ({
            id: producto.id,
            nombre: producto.nombre,
            codigo: producto.codigo,
            aplica_mayorista:
                producto.pivot?.aplica_mayorista ??
                producto.aplica_mayorista ??
                false,
            aplica_minorista:
                producto.pivot?.aplica_minorista ??
                producto.aplica_minorista ??
                false,
        }),
    );

    categoriasSeleccionadas.value = (props.promocion.categorias || []).map(
        (categoria) => ({
            id: categoria.id,
            nombre: categoria.nombre,
            aplica_mayorista:
                categoria.pivot?.aplica_mayorista ??
                categoria.aplica_mayorista ??
                false,
            aplica_minorista:
                categoria.pivot?.aplica_minorista ??
            ...
            <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                >
                    <div>
                        ...
                        >
                            Actualiza la oferta, fechas y alcance visualmente
                            sin perder la lógica actual.
                        </p>
                    </div>
